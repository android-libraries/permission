package com.github.shauway.permission;
...
public class PermissionManager {
    ...
    private void requestPermission(@NonNull Activity permissionManagerActivity, @NonNull String permission) {
        if (ContextCompat.checkSelfPermission(permissionManagerActivity, permission) != PackageManager.PERMISSION_GRANTED) {
            Log.i(TAG, "request permission: " + permission);//activity.permission.do.something.hidden.but.tong.wang.wong.
            ActivityCompat.requestPermissions(permissionManagerActivity, new String[]{permission}, 0);
        } else {
    ...
    protected void handlePermissionsResult(@NonNull final Activity permissionManagerActivity, @NonNull final String permissions[], int grantResults[]) {
        ...
        int unpromptCount = 0; // 用户拒绝并勾选[不再询问]的权限数
        ...
        if (unpromptCount > 0 && unpromptCount == permissions.length) {
            ...
            tvManualGrantPermissionTip.setText("去设置开启");
